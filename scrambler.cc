#define __MAKEMORE_SCRAMBLER_CC__ 1
...
Scrambler::Scrambler(const char *_dir, unsigned int _mbn) {
  mbn = _mbn;
  assert(mbn > 0);
...
void Scrambler::load_ctxtgt(FILE *infp) {
  int ret;
...
void Scrambler::present(double nu, double mu, double xi) {
  if (nu > 0 || mu > 0) {
    for (unsigned int mbi = 0; mbi < mbn; ++mbi) {
...
void Scrambler::load_ctx(FILE *infp) {
  int ret;
...
void Scrambler::generate(double dev, unsigned int reps) {
  for (unsigned int j = 0, jn = ctrlay->n * mbn; j < jn; ++j)
    fakectr[j] = sigmoid(randgauss() * dev);
  for (unsigned int mbi = 0; mbi < mbn; ++mbi) {
    encude( ctxbuf + mbi * ctxlay->n, ctxlay->n, cugenin + mbi * geninlay->n + 0);
    encude(fakectr + mbi * ctrlay->n, ctrlay->n, cugenin + mbi * geninlay->n + ctxlay->n);
  }
...
void Scrambler::report(const char *prog) {
  fprintf(
    stderr,
    "%s Scrambler %s rounds=%u\n"
